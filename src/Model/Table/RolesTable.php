...
class RolesTable extends Table
{
    ...
    public function initialize(array $config)
    {
        ...
        $this->setTable('roles');
        $this->setDisplayField('IdRoles');
        $this->setPrimaryKey('IdRoles');
    }
    ...
    public function validationDefault(Validator $validator)
    {
        $validator
            ->integer('IdRoles')
            ->allowEmptyString('IdRoles', null, 'create');

        $validator
            ->scalar('NombreRol')
            ->maxLength('NombreRol', 45)
            ->requirePresence('NombreRol', 'create')
            ->notEmptyString('NombreRol');
